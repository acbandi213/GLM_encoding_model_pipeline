...
class GLM_CV(GLM):
    def __init__(self, n_folds = 3, auto_split = True, split_by_group = True, split_random_state = None,
        ...
        self.skip_final_fold = skip_final_fold
    ...
    def fit(self, X, Y, train_idx = None, val_idx = None, group_idx = None, initial_w = None, initial_w0 = None,
        ...
        for n_fold in range(self.n_folds+1):  # when n_fold == self.n_folds, the fit is using all datapoints
            ...
            # The final full-data fold (n_fold == n_folds) warm-starts from CV fold weights that
            # were optimised on 2/3 of the data. On the full dataset the loss landscape shifts,
            # causing the warm-started weights to diverge regardless of learning rate.
            # When skip_final_fold=True, reuse fold 0's converged weights instead of refitting.
            if n_fold == self.n_folds and self.skip_final_fold:
                self.w_series_dict[n_fold] = self.w_series_dict[0]
                if verbose:
                    print('n_fold = {} skipped (skip_final_fold=True) — using fold 0 weights.'.format(n_fold))
                continue
